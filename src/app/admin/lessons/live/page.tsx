...
import { ArrowLeft, BookOpen, CheckCircle, Clock, FileCheck2, Filter, Globe, Plus, Search } from 'lucide-react';
  ...
  const renderTestInsertionControl = (index: number) => (
    <div className="flex items-center gap-3 py-1" key={`insert-${index}`}>
      ...
      <Button
        ...
        size="sm"
        variant="ghost"
        className="h-7 rounded-full border border-dashed border-indigo-300 px-3 text-xs text-indigo-800 hover:bg-indigo-50"
          ...
          <div className="flex items-center gap-4">
            <Button asChild variant="ghost">
              <Link
                href="/admin"
                onClick={event => {
                  if (pathDirty && !window.confirm(UNSAVED_PATH_MESSAGE)) {
                    event.preventDefault();
                  }
                }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-serif tracking-wide">Manage Learning Delivery</h1>
            </div>
          </div>
        ...
        <Tabs
          ...
          {practiceTypes.map(type => (
            <TabsContent key={type} value={type} className="mt-5">
              ...
              <RomanCard className="mb-6">
                <RomanCardContent className="flex items-center gap-4 p-4">
