...
import { LessonTypeTabs, type LessonTypeTab } from '@/src/components/ui/admin/LessonTypeTabs';
import {
  CategoryFormDialog,
...
const PRACTICE_LESSON_TYPE_TABS: LessonTypeTab[] = PRACTICE_LESSON_TYPES.map(option => ({
  value: option.value,
  label: option.label,
}));
...
function PracticeCategoriesPage() {
  ...
  return (
    <>
      <AdminPage>
        <AdminPageHeader
          title="Manage Practice Categories"
          description="Create type-specific tags for practice lessons. One lesson may have several tags."
          actions={
            <Button
              type="button"
        ...
        <section className="rounded-xl border bg-white/70 p-4 shadow-sm sm:p-6" aria-labelledby="category-list-heading">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <div
                className="mt-3 inline-flex gap-1 rounded-xl border border-border/80 bg-white/80 p-1.5 shadow-sm"
                role="group"
                ...
                {(['active', 'archived'] as const).map(option => (
                  <button
                    ...
                    className={`h-9 rounded-lg border border-transparent px-3.5 text-xs font-medium transition-[background-color,color,box-shadow] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 ${
                      status === option
                        ? 'border-primary/20 bg-primary text-primary-foreground shadow-[0_2px_6px_hsl(var(--primary)/0.22)]'
                        : 'text-roman-stone hover:bg-roman-parchment hover:text-foreground'
                    }`}>
