...
function CreateLessonPage() {
  ...
  const saveCurrentDraftAndExit = async (destination = '/admin') => {
  ...
  const handleBrowserBackButton = (destination = '/admin') => {
    if (!currentLesson) return;
    ...
    setDialogState({
      ...
      onConfirm: () => void saveCurrentDraftAndExit(destination),
      onAlternate: () => {
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-roman-stone">
              ...
              {hasDraft && (
                <span className="ml-2 inline-flex items-center gap-1 rounded-full border border-roman-gold/35 bg-roman-gold/15 px-2 py-1 text-xs font-medium text-foreground shadow-[0_1px_2px_rgb(15_23_42/0.04)]">
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-roman-gold"></span>
                  Draft
                </span>
              )}
            </p>
