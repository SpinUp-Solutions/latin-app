...
import { ArrowLeft, Archive, Copy, Loader2, MoveRight, Save } from 'lucide-react';
...
const errorMessage = (error: unknown) =>
  (error as { data?: { error?: string } })?.data?.error ?? 'The change could not be saved.';
...
function MockOverviewPage({ params }: { params: Promise<{ mockId: string }> }) {
  ...
  useUnsavedNavigationGuard(dirty, 'You have unsaved mock test changes. Leave this page?');

  const navigate = (href: string) => {
    if (dirty && !window.confirm('You have unsaved mock test changes. Leave this page?')) return;
    router.push(href);
  };
  ...
  if (!mock || !version)
    return (
      <main className="p-8" role="alert">
        Mock test or its version is unavailable. Retry from the Mock Tests page.
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-roman-marble">
      <header className="flex items-center justify-between border-b bg-white p-3">
        <Button variant="ghost" onClick={() => navigate('/admin/mock-tests')}>
      ...
      <div className="container mx-auto max-w-6xl space-y-6 px-4 py-8">
        ...
        <div>
          <span className="rounded bg-indigo-100 px-2 py-1 text-xs font-semibold text-indigo-800">MOCK TEST</span>
          <h1 className="mt-3 font-serif text-3xl">{mock.title}</h1>
