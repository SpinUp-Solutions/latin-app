...
const vocabStatusConfig: Record<
  'completed' | 'available',
  {
    card: string;
    icon: string;
...
> = {
  ...
  available: {
    card: 'bg-gradient-to-br from-amber-400/10 via-amber-300/5 to-amber-100/10 border border-amber-400/20',
    icon: 'text-amber-600',
...
export default function VocabSidebar({ currentLessonId }: VocabSidebarProps) {
  ...
  if (isLoading) {
    return (
      <div className="w-80 flex-shrink-0 h-full flex flex-col bg-gradient-to-br from-amber-50 via-white to-amber-100/30 border-l border-amber-400/20 relative">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-l from-amber-400/20 to-amber-300/15 rounded-full mix-blend-multiply filter blur-2xl opacity-60"></div>
        </div>
        <div className="relative px-6 py-8 border-b border-amber-400/10 bg-white/40 backdrop-blur-sm flex-shrink-0">
          <div className="flex items-center gap-3 mb-2">
            <div className="relative h-12 w-12 bg-gradient-to-br from-amber-400/20 to-amber-300/10 rounded-xl flex items-center justify-center shadow-lg border border-amber-400/20">
              <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent rounded-xl"></div>
              <BookOpen className="h-6 w-6 text-amber-600 drop-shadow-lg relative" />
            </div>
            <div>
              <h3 className="text-2xl font-serif text-gray-800">Vocabulary</h3>
            </div>
          </div>
  ...
  return (
    <div className="w-80 flex-shrink-0 h-full flex flex-col bg-gradient-to-br from-amber-50 via-white to-amber-100/30 border-l border-amber-400/20 relative">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-l from-amber-400/20 to-amber-300/15 rounded-full mix-blend-multiply filter blur-2xl opacity-60"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-gradient-to-r from-amber-300/15 to-amber-200/10 rounded-full mix-blend-multiply filter blur-2xl opacity-60"></div>
      </div>
        ...
        {lessons.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <div className="relative h-16 w-16 bg-gradient-to-br from-amber-100 to-amber-50 rounded-2xl mx-auto mb-4 flex items-center justify-center shadow-lg">
              ...
              <BookOpen className="h-8 w-8 text-amber-400 relative" />
            </div>
            <p className="text-base text-gray-500">No vocabulary lessons available</p>
          </div>
        ) : (
          <div className="space-y-3 p-4">
            {lessons.map(lesson => {
              const status = lesson.status === 'completed' ? 'completed' : 'available';
              const config = vocabStatusConfig[status];
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${config.card}
                    ${isCurrentLesson ? 'ring-2 ring-amber-500 ring-offset-2' : ''}
                  `}>
                  <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent rounded-2xl"></div>
