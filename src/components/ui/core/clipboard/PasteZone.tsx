...
export const PasteZone: React.FC<PasteZoneProps> = ({ pageType, pageIndex, className = '' }) => {
  ...
  return (
    <div className={`border-2 border-dashed border-blue-300 rounded-lg p-3 bg-blue-50/50 hover:bg-blue-50 transition-colors ${className}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-blue-700">
          <Clipboard className="h-4 w-4" />
          <span>Paste: {itemTitle} ({itemType})</span>
        </div>
        <Button
          variant="outline"
          size="sm"
